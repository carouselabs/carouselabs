...
- Use the same font style as the reference: match its font family character (condensed or wide, geometric or humanist, serif or sans-serif), font weight, casing, and letter spacing as closely as possible — if the reference uses bold condensed uppercase headlines, every slide uses bold condensed uppercase headlines
- Use the same text colors as the reference: if headlines are near-black with a single accent-colored word or phrase, reproduce exactly that treatment on every slide — never recolor all of the text with the accent color
- Use the same background treatment as the reference: if the background is white or light with colored accents, keep it white or light — never flood the slide with the accent color
