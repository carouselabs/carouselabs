...
Look carefully at the reference image and write its actual visual attributes into the image prompt as specific, concrete values — the image model follows whatever scene you describe, so the description itself must match the reference:
- Background: state whether it is light or dark and name its actual color/tone, then describe the generated image's background to match it. A light reference background means you describe a light background — never invert it into a dark or color-flooded design.
...
- Accent color: name it, and describe it used in the same proportion as the reference — highlights on a few key words, icons, or details, never the dominant color of the design.
...
Every color, mood, and lighting choice you write in the prompt must stay consistent with these observed attributes.`
