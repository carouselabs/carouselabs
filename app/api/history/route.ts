...
// Thumbnail generations and Custom Posts have no Idea behind them (see
// app/api/thumbnail/generate/route.ts and app/api/content-hub/custom-post —
// both create the Post standalone), so they're fetched separately and merged
// in below, sorted by their own createdAt against the unpinned idea entries'
// lastVisitedAt.
export async function GET() {
  const user = await getCurrentUser()
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })

  const [rows, thumbnailPosts, customPosts] = await Promise.all([
    ...
    db.post.findMany({
      where: { userId: user.id, format: "CUSTOM" },
      orderBy: { createdAt: "desc" },
      select: { id: true, caption: true, imageUrls: true, createdAt: true },
    }),
  ])
  ...
  const customEntries: CustomPostHistoryEntry[] = customPosts.map((post) => ({
    kind: "custom",
    id: post.id,
  ...
  // unpinned ideas, every thumbnail, and every custom post — is merged into
  // one chronological block underneath, using lastVisitedAt for ideas and
  // createdAt for the other two as the common sort key.
  const pinnedIdeas = ideaEntries.filter((e) => e.isPinned)
  const unpinnedIdeas = ideaEntries.filter((e) => !e.isPinned)
  const rest: AnyHistoryEntry[] = [...unpinnedIdeas, ...thumbnailEntries, ...customEntries].sort((a, b) => {
