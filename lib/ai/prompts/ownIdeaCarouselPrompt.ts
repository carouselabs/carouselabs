...
You will receive the post title, the caption, a deep dive into the topic, a predefined slide outline, the EXACT total number of slides in that outline, a required canvas size and aspect ratio, and optionally an uploaded reference image that defines the visual style.

The slide outline is generated by another AI model before this step and defines only the structure of the carousel. It specifies the sequence and purpose of each slide (for example: Hook, Tip N, Tip N, Tip N, ..., CTA — the actual number of Tip slides varies carousel to carousel).
...
For the Slide Indicator section — CRITICAL, read carefully: the user message states the EXACT total slide count for this carousel as a fact (for example "Total Slide Count: 12"). Every single slide's Slide Indicator must read "Slide {this slide's number} of {that exact total}" using THAT SAME total number, character-for-character identical, on every slide in the carousel. Never count the outline yourself, never infer the total from context, never let this number vary between slides — copy the stated total verbatim into every slide's indicator. A carousel with a stated total of 12 must show "of 12" on slide 1, slide 2, slide 3, and every other slide through slide 12, with no exceptions.
...
Total Slide Count: ${totalSlideCount}
This carousel has EXACTLY ${totalSlideCount} slides — no more, no fewer. Every slide's Slide Indicator section must state this exact number as its total (e.g. "Slide 3 of ${totalSlideCount}"), identically on every slide. Do not recount the outline above or use any other number.
